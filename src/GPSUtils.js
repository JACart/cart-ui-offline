export function convertGeoToPixelBig(
    latitude,
    longitude,
...
//New Map
    ...
    var mapOffsetY =
        (worldMapWidth / 2) * Math.log((1 + Math.sin(mapLatBottomDegree)) / (1 - Math.sin(mapLatBottomDegree))) + 725
    var y =
...
export function convertGeoToPixel(
    ...
    var mapOffsetY =
        (worldMapWidth / 2) * Math.log((1 + Math.sin(mapLatBottomDegree)) / (1 - Math.sin(mapLatBottomDegree)))
    var y =
        mapHeight - ((worldMapWidth / 2) * Math.log((1 + Math.sin(latitude)) / (1 - Math.sin(latitude))) - mapOffsetY) + 150
